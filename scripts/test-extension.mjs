...
test("Chrome release manifest keeps the minimal activeTab permission model", () => {
  ...
  assert.ok(
    ...
    `description has ${manifest.description.length} characters`,
  );
    ...
    const visit = (directory) => {
      for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) visit(entryPath);
          ...
          files.push(
            path.relative(browserDir, entryPath).split(path.sep).join("/"),
          );
      }
    };
    visit(browserDir);
...
test("shared model accepts only web URLs and normalizes persisted clickers", () => {
  ...
  const source = fs.readFileSync(
    path.join(rootDir, "src", "shared", "model.js"),
    "utf8",
  );
  vm.runInNewContext(source, context);

  assert.equal(
    context.KoalaClickerModel.parseSiteUrl("https://example.com/path").origin,
    "https://example.com",
  );
...
test("all published version sources stay synchronized", () => {
  ...
  assert.equal(
    readJson(path.join(rootDir, "package.json")).version,
    manifestVersion,
  );
  assert.equal(
    readJson(path.join(rootDir, "package-lock.json")).version,
    manifestVersion,
  );
  assert.equal(
    readJson(path.join(rootDir, "package-lock.json")).packages[""].version,
    manifestVersion,
  );

  const website = fs.readFileSync(
    path.join(rootDir, "website", "index.html"),
    "utf8",
  );
  assert.equal(
    website.match(
