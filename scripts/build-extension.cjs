  ...
  for (const item of fs.readdirSync(source)) {
    ...
    if (stat.isDirectory()) {
      ...
      if (stat.isSymbolicLink())
        throw new Error(
          `Symlinks are not allowed in build inputs: ${sourcePath}`,
        );
      if (/\.(?:js|css|html|json|xml|txt|md)$/.test(item))
        fs.writeFileSync(
          targetPath,
...
function validateManifest(manifest, browserName) {
  ...
  for (const permission of requiredPermissions) {
    if (!manifest.permissions.includes(permission)) {
      throw new Error(
        `${browserName}: missing required permission "${permission}"`,
      );
    }
  }
  ...
  if (manifest.host_permissions && manifest.host_permissions.length > 0) {
    throw new Error(
      `${browserName}: host_permissions should stay empty for activeTab-only access`,
    );
  }

  if (manifest.content_scripts) {
    throw new Error(
      `${browserName}: static content_scripts would weaken the activeTab privacy model`,
    );
  }

  if (manifest.web_accessible_resources) {
    throw new Error(
      `${browserName}: web_accessible_resources should stay empty unless a page must fetch extension assets`,
    );
  }
}
...
function writeManifest(targetDir, browserName, modifier) {
  ...
  if (releaseVersion && releaseVersion.replace(/^v/, "") !== manifest.version)
    throw new Error(
      "Release version must already match source; build never rewrites it.",
    );
  if (
    ...
    const addFiles = (directory) => {
      for (const item of fs
        ...
        if (item.isDirectory()) addFiles(itemPath);
          ...
          archive.append(fs.readFileSync(itemPath), {
...
async function buildBrowser(browserName, modifier) {
  ...
  const zipPath = path.join(
    distDir,
    `koalaclicker-${browserName}-v${version}.zip`,
  );
  await zipDirectory(targetDir, zipPath);

  console.log(
    `Built ${browserName} v${version}: ${path.relative(rootDir, zipPath)}`,
  );
}
...
async function run() {
  ...
  await buildBrowser("firefox", (manifest) => {
    manifest.background = { scripts: ["shared/model.js", "background.js"] };
  ...
  copyDir(path.join(rootDir, "website"), path.join(distDir, "website"));
  await zipDirectory(
    path.join(distDir, "website"),
    path.join(distDir, `koalaclicker-website-v${readManifest().version}.zip`),
  );
  console.log(`Built website v${readManifest().version}: dist/website`);
}
