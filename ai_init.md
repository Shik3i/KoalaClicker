...
1. **Zero Dependencies**: No runtime libraries or frameworks are shipped. The extension is vanilla HTML, CSS and JavaScript. npm dependencies are permitted for build, browser tests and validation.
...
2. **Review the Data Flow**: Understand that the Content Script is _stateless_. The Popup acts as the orchestrator. If the user reloads the page, the Content Script dies. Opening the popup is the _only_ way to re-inject the script and resume the auto-clickers.
...
4. **Inspect the CSS Selectors**: Look at `selectorFor` in `src/content/content.js`. Any modifications to how elements are identified must remain robust across page reloads.
